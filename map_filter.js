...
let uppercaseCats = cats.map( function(cat) {  //create a new array with the same names, but in uppercase. Original array is not changed
...
let uppercaseCats2 = cats.map( (cat) => {  //another way to code the creation of a new array (same as above, with slightly different notation)
    return cat.toUpperCase()
})
